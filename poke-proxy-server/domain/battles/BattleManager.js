 ...
 * 
 * 职责：
 * - 统一管理真人对战（PvP）和 AI 对战
 * - 创建和管理 Pokemon Showdown 对战引擎实例
 * - 处理协议路由和转发
 * - 管理玩家和 AI 的选择处理器
 * - 记录协议日志用于调试
 * 
 * 架构：
 * - 使用 Pokemon Showdown 的 BattleStream 作为对战引擎
 * - 使用 ProtocolRouter 路由协议到对应的处理器
 * - 使用 PlayerChoiceHandler 处理玩家选择
 * - 使用 AIChoiceHandler 处理 AI 选择
 * 
 * 支持的模式：
 * - 'ai': AI对战模式，p1 是玩家，p2 是 AI
 * - 'pvp': 真人对战模式，p1 和 p2 都是玩家
 */
const createLogger = require('../../adapters/logging/Logger');
const logger = createLogger('BattleManager');
...
class BattleManager {
  ...
  async initialize(team1, team2, formatid = 'gen9ou', seed = null) {
    logger.info(`初始化对战，模式: ${this.mode}`);
    
    // 使用适配层创建对战引擎
    const showdownAdapter = require('../../adapters/pokemon-showdown/ShowdownAdapter');
    const battleStream = showdownAdapter.createBattleStream();
    const streams = showdownAdapter.getPlayerStreams(battleStream);
    ...
    // 创建处理器
    const PlayerChoiceHandler = require('../choices/PlayerChoiceHandler');
    const AIChoiceHandler = require('../choices/AIChoiceHandler');
    ...
    // 创建协议路由器
    const ProtocolRouter = require('../choices/ProtocolRouter');
    this.router = new ProtocolRouter(this);

    // 准备初始化命令（但不立即发送，等待连接添加后再发送）
    // 注意：showdownAdapter 和 Teams 已在方法开头声明
...
>player p2 ${JSON.stringify({name: this.mode === 'ai' ? 'AI' : 'Player 2', team: p2Team})}`;
   ...
   */
  cleanup() {
    logger.info('清理 BattleManager 资源');
    
    // 清理流
    if (this.engine) {
      try {
        if (this.engine.omniscient && typeof this.engine.omniscient.destroy === 'function') {
          this.engine.omniscient.destroy();
        }
        if (this.engine.p1 && typeof this.engine.p1.destroy === 'function') {
          this.engine.p1.destroy();
        }
        if (this.engine.p2 && typeof this.engine.p2.destroy === 'function') {
          this.engine.p2.destroy();
        }
      } catch (e) {
        logger.error('清理流时出错', e);
      }
    }
    ...
    // 重置标志
    this._omniscientListenerRunning = false;
    this._initCommandsSent = false;
    
    logger.info('BattleManager 资源已清理');
  }
